...
def find_quest(name):
    ...
    cn_data = dict(re.findall(r'"([a-f0-9]+)":\s*"([^"]+)"', h[idx:end]))
    idx2 = h.find("const QUEST_MARKERS = [")
    end2 = h.find("];", idx2)
    qm = []
    for m in re.finditer(r'{([^}]+)}', h[idx2:end2]):
        try: qm.append(json.loads("{" + m.group(1) + "}"))
        except: pass
    tid = None
    for qid, cn_name in cn_data.items():
        if name.lower() in cn_name.lower(): tid = qid; break
    if not tid:
        for entry in qm:
            if name.lower() in entry.get("name", "").lower(): tid = entry["id"]; break
    if not tid: print(f"Quest '{name}' not found!"); sys.exit(1)
...
"""
            ...
            print(f"  {result['markers']} markers, size {result['w']}x{result['h']}", end="", flush=True)
            time.sleep(0.5)
...
if __name__ == "__main__":
    if len(sys.argv) < 2:
        print("Usage: python scripts/screenshot_quest.py 'task name'")
        sys.exit(1)
